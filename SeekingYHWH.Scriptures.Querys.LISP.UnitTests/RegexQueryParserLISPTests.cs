...
{
	public class RegexQueryParserLISPTests
	{
		#region Class Fields
		...
		{
			var parser = new QueryParserLISPParser(parsers, "Regex(pattern)");
			var parsed = parser.TryParse(out var provider);
			Assert.True(parsed);
			var actual = provider as RegexQueryProvider;
		...
		{
			var parser = new QueryParserLISPParser(parsers, "Regex(pattern, IgnoreCase)");
			var parsed = parser.TryParse(out var provider);
			Assert.True(parsed);
			var actual = provider as RegexQueryProvider;
			Assert.NotNull(actual);
