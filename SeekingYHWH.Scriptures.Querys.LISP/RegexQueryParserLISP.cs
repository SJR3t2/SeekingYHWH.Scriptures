...
{
	public sealed class RegexQueryParserLISP : QueryParserLISP
	{
		#region Constants
		...
		#region Class Fields
		private static readonly Dictionary<string, RegexOptions> optionss = new Dictionary<string, RegexOptions>(StringComparer.InvariantCultureIgnoreCase)
		{
		...
		{
			return new RegexQueryParserLISP(parser);
		}
		#endregion //Class Methods

		#region Constructors
		public RegexQueryParserLISP(QueryParserLISPParser parser)
			: base(parser)
		{
		}
		...
		public override bool TryParse(out QueryProvider provider)
		{
			...
			if (!parser.NextToken(out var token))
			{
				...
				provider = new RegexQueryProvider(pattern, RegexOptions.IgnoreCase);
				return true;
			}
			if (token == ')')
			{
